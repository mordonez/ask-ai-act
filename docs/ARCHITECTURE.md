...
**Dónde vive el corpus del Reglamento — 🟢 orientación decidida, aunque el resto de la fase siga abierta:** en el cliente, como asset estático servido aparte del bundle principal (no en el JS de arranque del wizard), no en un backend. Coherente con que Fase 1 ya es 100% cliente (sin cuentas, sin base de datos) y con que la otra opción ya evaluada más abajo para esta fase (WebLLM) implica de todas formas tener el corpus disponible en el navegador. El texto (`sources/reglamento_ue_2024_1689_es.txt`, ~800 KB, comprime bien) es pequeño frente al peso de un modelo WebLLM (cientos de MB a varios GB), así que si esa opción ya se da por aceptable, el corpus en cliente lo es con más razón. Esto no decide el resto: ni si el LLM que lo usa corre en el navegador o en servidor, ni el *chunking*, ni el indexado — solo fija que el texto de origen no depende de una llamada a servidor para estar disponible. Nota importante si se usa para citar textualmente: es el texto **original** de 2024/1689, no la versión consolidada con modificaciones posteriores (ver nota de procedencia en `sources/README.md`) — cualquier UI que lo muestre debe conservar ese aviso.
