...
- ✅ Tests automáticos (Vitest, `npm test`) que verifican que los 5 casos de uso reales de la guía 2 de AESIA clasifican como "alto riesgo" con el Anexo/apartado exacto citado en el PDF. 73 tests en verde.
- ✅ Enlaces por pregunta (`links`) al artículo/anexo exacto en artificialintelligenceact.eu, verificados que responden 200 antes de publicarlos — y una sección "Para profundizar" (`src/rules/resources.ts`) con 6 recursos generales (AI Act Explorer, comprobador de cumplimiento, resumen de alto nivel, gobernanza y aplicación de la Comisión, plan de acción de ciberseguridad e IA, informe del CGPJ sobre la ley española). Todas las preguntas menos una (la 9ª prohibición, del Reglamento 2026/1744, sin página dedicada en ese sitio) tienen al menos un enlace.
- ✅ Ejemplos de "sí" y "no" en listas separadas por color (verde/rojo) en vez de un párrafo, con 2 ejemplos por lado en casi todas las preguntas (antes 1+1) — para que la diferencia se vea de un vistazo, no leyendo un párrafo entero.
...
- ✅ UI del wizard desplegada en [aiact.miguelordonez.com](https://aiact.miguelordonez.com), probada de principio a fin en navegador (los 3 caminos: alto riesgo con rol y plan, uso prohibido sin pregunta de rol, y el árbol completo de las 9 prácticas prohibidas una a una).
...
- ✅ Bloque de contexto plegable "¿Qué es el Reglamento de IA en 2 minutos?" (`introHtml()` en `wizard.ts`), antes de la primera pregunta: los 6 resultados posibles explicados en una frase cada uno (mismos `ClassificationLabel` que usa el árbol, no categorías nuevas), qué NO hace la herramienta, y una sola fecha (art. 113) con enlace al texto oficial en EUR-Lex en vez de una tabla de hitos que se desactualiza. Pensado para que alguien sin contexto previo del Reglamento entienda qué está a punto de responder sin salir de la app — motivado al comparar con dossiers externos tipo aisummitbarcelona.com/resources/eu-ai-act-explained, que sí mantienen tablas de fechas (riesgo de quedar desactualizadas).
- ✅ Auditoría de cobertura del árbol de preguntas contra el texto oficial completo (`eval/context/arbol-preguntas.md` + `sources/reglamento_ue_2024_1689_es.txt`, ver `eval/README.md`), y las correcciones que salieron de ella:
  - Las obligaciones de transparencia del art. 50 estaban incompletas y parcialmente conflacionadas. Ahora son cuatro preguntas/ramas independientes: 50.1 (interacción, sin cambios), 50.2 (marcado técnico del contenido generado, ya no exige que se publique — antes sí, y eso dejaba sin detectar la obligación de un proveedor con contenido generado pero no publicado), 50.4 (divulgación al publicar, con dos regímenes de excepción distintos según si el contenido es texto —revisión editorial— o imagen/audio/vídeo —sin excepción de revisión editorial en el texto legal, así que no se ofrece—), y 50.3, pregunta nueva (categorización biométrica o reconocimiento de emociones expuestos a personas físicas), un hueco real que no tenía ninguna pregunta antes.
  - El caso real de la universidad (revisión editorial sustantiva) ya no exime de todo el art. 50: exime de la divulgación pública (50.4), pero el marcado técnico (50.2) sigue aplicando — esa excepción es del 50.4, no existe para el 50.2 en el texto legal.
  - Ejemplo corregido en `prohibido_biometria_remota_tiempo_real`: la identificación biométrica diferida no es una "excepción tasada" del art. 5.1.h, cae fuera de ese artículo por no ser en tiempo real (régimen distinto, Anexo III).
  - 11 preguntas nuevas de tests en `tests/classify.test.ts` y `tests/actionPlan.test.ts` cubriendo las ramas nuevas y sus "no lo sé" — 84 tests en verde (antes 73).
...
- ✅ Nombre de empresa/sistema (opcional): primer paso antes de la primera pregunta, con "Prefiero no decirlo" para saltarlo. Identifica cada entrada del histórico ("Acme S.L. — Sistema de alto riesgo" en vez de solo la etiqueta) y aparece como subtítulo del resultado. Texto libre, así que se escapa explícitamente (`escapeHtml()`) antes de insertarlo en el HTML — probado en navegador con un intento de inyección de `<script>` en el nombre: se muestra como texto literal, no se ejecuta.
