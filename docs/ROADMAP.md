...
**Experimento de validación (3 sept 2026):** se probó dar la descripción libre de 3 casos (incluido uno diseñado como trampa de falso positivo) a un agente sin contexto del árbol, para ver si un modelo por sí solo clasificaba bien sin necesidad del wizard. Los 3 acertaron, y en 2 de los 3 el agente encontró matices que el árbol no tenía — uno de ellos (la excepción del art. 50.4) ya está corregido arriba. El otro (que la biometría laboral puede encajar a la vez en Anexo III 1(a) *y* 4(b)) no cambia ningún resultado hoy — el árbol ya concluye alto riesgo correctamente por la vía de la pregunta única de Anexo I/III — pero confirma el punto ya anotado más abajo sobre granularizar esa pregunta. El experimento no valida sustituir el wizard por un prompt único (falta probar repetibilidad y casos adversos), pero sí refuerza la Fase 3 tal como está planteada: un modelo ayudando a rellenar el árbol, no decidiendo por su cuenta.
